...
function fetchPopulationData(countryId, token) {
  var populationDataFetcher = new service.EntityServiceAccess(
    countrySeparatedDataServiceHost, 
    countrySeparatedDataServicePort, 
...
function fetchIndicators(countryId, token) {
  var indicatorsFetcher = new service.EntityServiceAccess(
    countrySeparatedDataServiceHost, 
    countrySeparatedDataServicePort, 
    (countryId) => 'countries/' + countryId + '/indicators' // endpointBuilder
  )
  return indicatorsFetcher
      .setToken(token).setEntityId(countryId).accessService()  
}
    ...
    .then(function(plainToken) {
      try {
        ...
        Promise.all([
          ...
          fetchIndicators(countryId, authToken)
        ])
        .spread(function(populationData, indicatorData) {
          // assemble data and give response
          countryData.mainCities = mainCitiesData
          countryData.population = populationData
          countryData.indicators = indicatorData
          response.status(200)
